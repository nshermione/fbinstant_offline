...
export class CString {
  ...
  static formatDayHourMinSec(totalSec: number) {
    totalSec = Math.floor(totalSec);
    let day = Math.floor(totalSec / (60 * 60 * 24));
    let hour = Math.floor((totalSec % (60 * 60 * 24)) / (60 * 60));
    let minute = Math.floor((totalSec % (60 * 60)) / 60);
