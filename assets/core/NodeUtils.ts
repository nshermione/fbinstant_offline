...
export class NodeUtils {
  ...
  static findNode(root: cc.Node, nodeName: string, requireActive = false) {
    if (requireActive && root.active == false) {
      return;
    }
  ...
  static setLabel(rootNode, nodeName, value, options: ILocaleLabel = {}) {
    let node = NodeUtils.findNode(rootNode, nodeName, options.requireActive);
    if (node) {
